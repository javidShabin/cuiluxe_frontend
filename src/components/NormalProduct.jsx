...
const NormalProduct = () => {
  ...
  const fetchProducts = async (page = 1) => {
    try {
      ...
      const res = await axiosInstance.get(
        `/product/get-package-products?page=${page}&limit=${itemsPerPage}`
      );
      ...
      const allRes = await axiosInstance.get("/product/get-package-products?page=1&limit=1000");
  ...
  const selectType = async (type, page = 1) => {
    try {
      ...
      const allRes = await axiosInstance.get("/product/get-package-products?page=1&limit=1000");
      const allProductsData = allRes.data?.products || [];
      const filteredProducts = allProductsData.filter(p => p.types === type);
      
      const skip = (page - 1) * itemsPerPage;
      const paginatedProducts = filteredProducts.slice(skip, skip + itemsPerPage);
      const totalPagesForFilter = Math.ceil(filteredProducts.length / itemsPerPage);

      setProducts(paginatedProducts);
      setCurrentPage(page);
      setTotalPages(totalPagesForFilter);
  ...
  const fetchProductsByCategory = async (category, page = 1) => {
    try {
      ...
      const allRes = await axiosInstance.get("/product/get-package-products?page=1&limit=1000");
      const allProductsData = allRes.data?.products || [];
      let filteredProducts = allProductsData.filter(p => p.category === category);
      
      if (selectedType) {
        filteredProducts = filteredProducts.filter(p => p.types === selectedType);
      }
